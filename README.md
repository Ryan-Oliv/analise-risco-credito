...
contém um subconjunto de 104.805 registros.
...
`DebtRatio` indica problema sistemático de escala ou de cálculo na origem, não
outliers isolados — o tratamento será definido na etapa de EDA.

### Variáveis priorizadas para a modelagem

`NumberOfTimes90DaysLate` · `NumberOfOpenCreditLinesAndLoans` · `age` ·
...
Aprofundamento da EDA com Pandas e visualizações, tratamento dos nulos e das escalas
anômalas identificadas na etapa anterior, e engenharia de atributos.

### 3. Modelo de classificação — planejado

Modelo em scikit-learn para prever `SeriousDlqin2yrs`, com atenção ao desbalanceamento
da base. Avaliação por AUC-ROC, matriz de confusão e análise de precisão x recall.
